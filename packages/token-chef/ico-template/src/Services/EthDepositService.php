...
class EthDepositService
{
    ...
    {
        $deposit = CryptoDeposit::whereDepositKind(StaticArray::CRYPTO_DEPOSIT_KIND_ETH)->whereUserId($user->id)->first();
        if (!$deposit) {
    ...
    {
        $deposit = CryptoDeposit::whereDepositKind(StaticArray::CRYPTO_DEPOSIT_KIND_ETH)->whereNull('user_id')->first();
        if (!$deposit) {
            throw new WebException(trans('ico-template::home.all_eth_deposits_taken'));
        }
